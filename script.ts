...
// Интерфейс для фигуры тетромино
interface Tetrimino {
    shape: number[][];
    color: string;
}
...
// Задаем тетромино
const shapes: { [key: string]: Tetrimino } = {
    I: { shape: [[1], 
                 [1], 
                 [1], 
                 [1]], 
...
let currentShape: Tetrimino = shapes[shapeKey];
...
// Рисуем фигуру
function drawShape(shape: Tetrimino): void {
...
function handleSpacebarPress(event: KeyboardEvent): void {
    if (event.code === 'Space') {
        currentShape.shape = rotateShape(currentShape.shape);
        // Очищаем поле и перерисовываем фигуру
        const cells = tetrisPlaygroundTarget.querySelectorAll('.cell');
        cells.forEach(cell => {
            (cell as HTMLElement).style.backgroundColor = ''; // Очищаем цвет клеток
        });
